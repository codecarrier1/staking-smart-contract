...
contract TOKEN is Context, IBEP20, Ownable {
    ...
    /**
     * @dev Returns if the given address is added as a PancakeSwap pool address
     */
    function isSwapAddress(address addressValue) external view returns (bool) {
        return _swapAddresses[addressValue];
    }

    /**
     * @dev Add an address as the PancakeSwap pool address
     */
    function addSwapAddress(address swapAddress)
        external
        onlyOwner
        returns (bool)
    {
     ...
     * @dev Add an address as the PancakeSwap pool address
     */
    function resetSwapAddresses() external onlyOwner returns (bool) {
        for (uint256 i = 0; i < _swapAddressesArr.length; i++) {
